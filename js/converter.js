/**
 * converter.js
 * Unit conversion for the on-screen converter panel.
 *
 * Handles the converter portion of the UI: quantity selector, from/to unit
 * selects, the switch button, and live-updating the result line as the
 * display value changes. Temperature uses affine conversions; currency uses
 * live exchange rates fetched from the Frankfurter API (EUR base), falling
 * back to cached/static rates when offline.
 *
 * Exposes window.Converter with:
 *   - setQuantity(q):  activate one converter container
 *   - convert():       recompute the result for the active container
 *   - init():          bind all converter wiring
 */
...
const conversionTable = {
  ...
  function formatUnitName(quantity, key) {
    // Display friendly labels; currency shows the uppercase ISO code.
    if (quantity === 'currency') return key.toUpperCase();
    return key;
  }

  /* ------------------------- Conversions ------------------------------- */

  // value arrives as a number; quantity/from/to are strings.
  function doConvert(quantity, from, to, value) {
    if (quantity === 'temperature') {
      return temperatureConvert(from, to, value);
    }
    if (quantity === 'currency') {
      return currencyConvert(from, to, value);
    }
    const row = conversionTable[quantity] && conversionTable[quantity][from];
    if (row && typeof row[to] === 'number') {
      return value * row[to];
    }
    return NaN;
  }

  function temperatureConvert(from, to, value) {
    // helper: celsius intermediate
    const toCelsius = {
      celsius: (v) => v,
    ...
    const c = toCelsius[from];
    const out = fromCelsius[to];
    if (!c || !out) return NaN;
    return out(c(value));
  }

  // Static fallback rates (per EUR) for currencies the Frankfurter API does
  // not report. Current as of Sept 2026; used to complete coverage.
  const STATIC_EUR_RATES = {
    rub: 100.0,
    aed: 4.265,
  };

  // Live currency rates via the Frankfurter API (free, keyless, EUR base).
  // ISO codes are normalized to lowercase to match the HTML option values.
  function currencyConvert(from, to, value) {
  ...
  async function refreshCurrencyCache() {
    try {
      ...
      Object.keys(data.rates || {}).forEach((code) => {
        rates[code.toLowerCase()] = data.rates[code];
      });
      // Frankfurter omits a few currencies; fill them from static fallbacks.
      Object.keys(STATIC_EUR_RATES).forEach((code) => {
        if (!(code in rates)) rates[code] = STATIC_EUR_RATES[code];
      });
      rateCache = rates;
      currencyFetched = true;
      // fire an update so the visible result refreshes with fresh rates
      convert();
    } catch (err) {
      // keep whatever we have cached; leave a hint in the console
    }
  }
  ...
  function displayValue() {
    ...
    if (!isNaN(plain) && String(plain).length >= raw.length && /^[0-9.eE+\\-]+$/.test(raw)) {
      return plain;
    }
  ...
  function convert() {
    ...
    const result = doConvert(selectedQuantity, from, to, value);
    resultEl.textContent = formatNumber(value) + ' ' + formatUnitName(selectedQuantity, from) +
      ' = ' + formatNumber(result) + ' ' + formatUnitName(selectedQuantity, to);
  }

  function setQuantity(quantity) {
    selectedQuantity = quantity;
    // deactivate the currently active container, activate the new one
    document.querySelectorAll('.converter-container').forEach((container) => {
      const active = container.classList.contains(quantity);
      container.classList.toggle('converter-hidden', !active);
      container.classList.toggle('converter-active', active);
    });
    // Currency needs live rates; fetch on first use, then repaint.
    if (quantity === 'currency' && !currencyFetched) {
      refreshCurrencyCache(); // calls convert() when rates arrive
      return;
    }
    convert();
  }
  ...
  function init() {
    ...
    // live conversion as the display value changes (typing or calculator buttons)
    displayEl.addEventListener('input', convert);
    document.addEventListener('calculator:display', convert);

    // kick off the initial currency fetch so rates are ready when the user
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = global.Converter;
}